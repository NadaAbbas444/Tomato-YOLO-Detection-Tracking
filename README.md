...
|-------------------------------|-----------------------------------------------------------------------------|------------------------------------------------------------------|---------------------------------------------------------------------------------|
| `detect.py`                   | Runs YOLOv5 model on custom images and saves detection results.             | After training the model or for simple detection without counting.| `python scripts/detect.py --weights /path/to/weights/best.pt --source /path/to/testing_images/ --save-txt --save-conf --project /path/to/save_results/` |
| `detect_n_count.py`           | Detects tomatoes and counts them without tracking across frames.            | Use this for counting tomatoes in single images or non-sequential image sets. | `python scripts/detect_n_count.py --weights /path/to/weights/best.pt --source /path/to/testing_images/ --save-txt --save-conf --project /path/to/save_results/` |
| `detect_n_count_n_track.py`   | Detects, counts, and tracks tomatoes across consecutive frames.             | Use this when tracking tomatoes across video frames or sequential images is required. | `python scripts/detect_n_count_n_track.py --weights /path/to/weights/best.pt --source /path/to/testing_images/ --save-txt --save-conf --project /path/to/save_results/` |
...
<p align="center">
  <img src="runs/train/exp/F1_curve.png" alt="F1-Confidence Curve" width="45%" />
  <img src="runs/train/exp/PR_curve.png" alt="Precision-Recall Curve" width="45%" />
</p>
